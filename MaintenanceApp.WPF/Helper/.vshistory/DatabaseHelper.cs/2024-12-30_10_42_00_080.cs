...
public class DatabaseHelper : IDatabaseHelper
{
    ...
    public async Task<IEnumerable<dynamic>> GetFilteredLinkedRecordsAsync(string table1 ,string table2 ,string joinColumn ,string filterValue)
    {
        try
        {
            using var connection = CreateConnection();
            ...
            WHERE 
                v.{joinColumn} LIKE @FilterValue";

            return await connection.QueryAsync(query ,new { FilterValue = $"%{filterValue}%" });
        }
        catch(Exception ex)
        {
            MessageBox.Show($"Error DatabaseHelper.GetFilteredLinkedRecordsAsync: {ex.Message}");
            return null;
        }
    }

    public Task<IEnumerable<dynamic>> GetFilteredLinkedRecordsAsync<T>(string table1 ,string table2 ,string joinColumn ,string filterValue) where T : class
    {
