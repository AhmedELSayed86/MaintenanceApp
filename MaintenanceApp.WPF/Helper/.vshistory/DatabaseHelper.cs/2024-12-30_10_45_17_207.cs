...
public class DatabaseHelper : IDatabaseHelper
{
    ...
    public async Task<IEnumerable<VisitData>> GetFilteredLinkedRecordsAsync<T>(string table1 ,string table2 ,string joinColumn ,string filterValue) where T : class
    {
        try
        {
            using var connection = CreateConnection();
            ...
            return await connection.QueryAsync<VisitData>(query ,new { FilterValue = $"%{filterValue}%" });
        }
        catch(Exception ex)
        {
