...
public class DatabaseHelper : IDatabaseHelper
{
    ...
    public async Task<IEnumerable<LinkedVisitData>> GetFilteredLinkedRecordsAsync(
       string table1 ,string table2 ,string table3 ,string joinColumn1 ,string joinColumn2 ,string filterValue)
    {
        try
        {
            using var connection = CreateConnection();
            ...
            FROM 
                {table1} v
            INNER JOIN 
                {table2} s ON v.{joinColumn1} = s.{joinColumn1}
            INNER JOIN 
                {table3} e ON v.Technician = e.EmployeeID
            WHERE 
                v.{joinColumn1} LIKE @FilterValue";

            return await connection.QueryAsync<LinkedVisitData>(query ,new { FilterValue = $"%{filterValue}%" });
        }
        catch(Exception ex)
        {
