        ...
        var constraintQueries = new List<string>
            {
                ...
                "ALTER TABLE Users ADD COLUMN IsActive INTEGER;",
                "ALTER TABLE Users ADD COLUMN DeactivatedOn DATETIME;"
    ...
    public async Task<bool> AddTestUserAsync()
    {
        try
        {
            await Task.Run(() =>
          {
              ...
              if(isuser.Any())
              {
                  MessageBox.Show($"Ahmed was found: {isuser.FirstOrDefault().UserName}");
                  return;
              }
              else
              {
                  MessageBox.Show("Ahmed was not found");
              }
          });
            ...
            var newUser = new User
            {
                ...
                ChangeOn = System.DateTime.UtcNow ,
                CreatedBy = 1 ,

                Notes = "تم الانشاء بواسطةالنظام"
            };

            var v = await InsertRecordAsync(newUser);
            MessageBox.Show($"Ahmed was Created IsCompleted: {v.IsCompleted.ToString()}.\n" +
                            $"Ahmed was Created IsCanceled: {v.IsCanceled.ToString()}.\n" +
                            $"Ahmed was Created IsFaulted: {v.IsFaulted.ToString()}.\n");
