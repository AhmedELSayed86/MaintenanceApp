...
public class DatabaseHelper : IDatabaseHelper
{
    ...
    private void CreateTables()
    {
        ...
        string createStatusDatasTable = @"CREATE TABLE IF NOT EXISTS StatusDatas ( Code INTEGER PRIMARY KEY AUTOINCREMENT, Name TEXT NULL, Job TEXT NULL,  Branch TEXT NULL, WorkLocation TEXT NULL, Department TEXT NULL, Vendor INTEGER NULL,  CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER, ChangeBy INTEGER, Notes TEXT );";
        ...
        ExecuteNonQuery(connection ,createStatusDatasTable);
    ...
    public async Task<IEnumerable<dynamic>> GetSparePartsWithTechnicianAsync()
    {
        return await Task.Run(() =>
        {
            ...
            string query = @"
                ...
                INNER JOIN 
                    StatusDatas sp ON t.PartId = sp.Code
                INNER JOIN 
    ...
    public async Task<bool> AddTestUserAsync()
    {
        try
        {
            ...
            if(userExists == null)
            {
                ...
                MessageController.SummaryAsync($"تمت إضافة المستخدم التجريبي بنجاح.");
            }
            return true;
        }
        catch(Exception ex)
        {
            MessageController.SummaryAsync($"خطأ أثناء إضافة المستخدم التجريبي: {ex.Message}");
            return false;
        }
    }
