...
public class DatabaseHelper : IDatabaseHelper
{
    ...
    private async Task InitializeEncryptionAsync()
    {
        ...
        {
            await MessageController.SummaryAsync($"خطأ أثناء تهيئة التشفير: {ex.Message}" ,Brushes.IndianRed);
        }
    }
    ...
    private async Task CreateTablesAsync()
    {
        ...
        // بيانات العملاء من الساب
         @"CREATE TABLE IF NOT EXISTS SAPDatas (  Notification DOUBLE PRIMARY KEY, NotificationType TEXT, Region INTEGER, City TEXT, Street TEXT, ListName TEXT, Telephone TEXT, District TEXT, NotifDate TEXT, Description TEXT, Customer INTEGER, MainWorkCtr TEXT, SortField TEXT, BreakdownDuration INTEGER, RequiredEnd TEXT,StatusID INTEGER, CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER,  ChangeBy INTEGER, Notes TEXT );"
        // بيانات العملاء من الساب
      , @"CREATE TABLE IF NOT EXISTS Statuses (  ID INTEGER PRIMARY KEY, Name TEXT,SAPNO INTEGER,DiscriptionAR TEXT,DiscriptionEN TEXT, CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER, ChangeBy INTEGER, Notes TEXT );"
        // تتميم الفنيين لبلاغات الساب
        , @"CREATE TABLE IF NOT EXISTS VisitDatas ( ID INTEGER PRIMARY KEY, Notification DOUBLE,  VisitDate TEXT, Technician INTEGER, ServiceDetails TEXT, Implemented TEXT,  Paid TEXT, Unpaid TEXT, Cost REAL, PaymentRefused INTEGER, DistributionStatus TEXT, AssistantTechnician INTEGER, DeterminationTechnician INTEGER, InstallationDate TEXT, ExecutionDuration INTEGER, WarrantyStatus TEXT, UserStatus TEXT,  DiscountPercentage REAL,  CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER, ChangeBy INTEGER, Notes TEXT);"
        // بيانات قطع الغيار
        , @"CREATE TABLE IF NOT EXISTS SparePartDatas ( SapCode INTEGER PRIMARY KEY AUTOINCREMENT, PartNo TEXT, Group_ TEXT, Model TEXT, DescriptionAR TEXT, DescriptionEN TEXT, C1 TEXT, C2 TEXT, IsDamaged BOOLEAN, CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER, ChangeBy INTEGER, Notes TEXT );"
        // عهدة الفني
        , @"CREATE TABLE IF NOT EXISTS SparePartsAtTechnicianDatas ( ID INTEGER PRIMARY KEY, SapCode INTEGER, Technician INTEGER, CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER, ChangeBy INTEGER, Notes TEXT );"
        // قطع الغيار التي تم تركيبها على بلاغ معين
       , @"CREATE TABLE IF NOT EXISTS SparePartsInstalledDatas (  ID INTEGER PRIMARY KEY, Notification DOUBLE, SapCode INTEGER, Technician INTEGER, CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER, ChangeBy INTEGER, Notes TEXT);"
        // بيانات الموظفين والفنيين
       , @"CREATE TABLE IF NOT EXISTS EmployeeDatas ( Code INTEGER PRIMARY KEY AUTOINCREMENT, Name TEXT NULL, Job TEXT NULL,  Branch TEXT NULL, WorkLocation TEXT NULL, Department TEXT NULL, Vendor INTEGER NULL,  CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER, ChangeBy INTEGER, Notes TEXT );"
      ...
      , @"CREATE TABLE IF NOT EXISTS MyNotesDatas (ID INTEGER PRIMARY KEY AUTOINCREMENT, Title TEXT,              Content TEXT, AlertTime DATETIME, Alerted INTEGER, CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER,  ChangeBy INTEGER, Notes TEXT );" };
        ...
        await ExecuteNonQueryAsync(connection ,createSAPDatasTable);
        await ExecuteNonQueryAsync(connection ,createStatusesTable);
        await ExecuteNonQueryAsync(connection ,createVisitDatasTable);
        await ExecuteNonQueryAsync(connection ,createSparePartsInstalledDatasTable);
        await ExecuteNonQueryAsync(connection ,createEmployeeDatasTable);
        await ExecuteNonQueryAsync(connection ,createSparePartDatasTable);
        await ExecuteNonQueryAsync(connection ,createMyNotesDatasTable);
        await ExecuteNonQueryAsync(connection ,createSparePartsAtTechnicianDatasTable);
    }
