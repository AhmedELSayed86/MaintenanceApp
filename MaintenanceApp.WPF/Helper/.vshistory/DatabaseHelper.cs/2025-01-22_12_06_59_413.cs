   ...
   private async Task InitializeEncryptionAsync()
{
    try
    {
        // الحصول على كلمة المرور من مصدر آمن
        string password = await GetPasswordFromSecureSourceAsync();
...
private async Task<string> GetPasswordFromSecureSourceAsync()
{
    // الحصول على كلمة المرور من مصدر آمن (مثل Azure Key Vault أو ملف إعدادات مشفر)
    return await KeyVault.GetSecretAsync("DatabasePassword");
}
