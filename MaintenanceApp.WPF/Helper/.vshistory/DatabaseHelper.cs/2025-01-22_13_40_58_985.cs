...
public class DatabaseHelper : IDatabaseHelper
{
    ...
    public DatabaseHelper()
    {
        try
        {
            ...
            _connectionString = $"Data Source={dbFilePath};Version=3;Password={decryptedPassword};";
    ...
    private async Task ALTERTABLEAsync()
    {
        ...
        catch(SQLiteException ex)
        {
            ...
            await await MessageController.SummaryAsync($"Constraint Error: {ex.Message}" ,Brushes.IndianRed);
        }
        catch(Exception ex)
        {
            // قد تحدث الأخطاء إذا كانت القيود موجودة بالفعل
    ...
    public async Task<bool> AddTestUserAsync()
    {
        try
        {
            ...
            {
                await await MessageController.SummaryAsync("المستخدم موجود بالفعل.");
                return false;
            }
            ...
            await await MessageController.SummaryAsync("تمت إضافة المستخدم التجريبي بنجاح.");
            return true;
        }
        catch(Exception ex)
        {
            await await MessageController.SummaryAsync($"خطأ أثناء إضافة المستخدم التجريبي: {ex.Message}");
            return false;
        }
    }
