...
public class DatabaseHelper : IDatabaseHelper
{
    ...
    private async Task CreateTablesAsync()
    {
            ...
            var createTableQueries = new List<string>
        {
           ...
           @"CREATE TABLE IF NOT EXISTS MyNotesDatas ( ID INTEGER PRIMARY KEY AUTOINCREMENT, Title TEXT,              Content TEXT, AlertTime DATETIME, Alerted INTEGER, CreatedOn TEXT, ChangeOn TEXT, CreatedBy INTEGER,  ChangeBy INTEGER, Notes TEXT );"
            ...
            dbTarget.Parameters.Add(new DatabaseParameterInfo("@time" ,"${longdate}"));
            ...
            string insertQuery = @"
                ...
                VALUES (@Date, @Level, @Message, @Logger, @Exception)";

            using var command = new SQLiteCommand(insertQuery ,connection);
            command.Parameters.AddWithValue("@Date" ,DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            command.Parameters.AddWithValue("@Level" ,"Info");
            command.Parameters.AddWithValue("@Message" ,"Test log entry");
            command.Parameters.AddWithValue("@Logger" ,"TestLogger");
            command.Parameters.AddWithValue("@Exception" ,DBNull.Value);

            command.ExecuteNonQuery();
            Console.WriteLine("تم إدخال سجل جديد.");
    ...
    public async Task AddLogToDatabase(string level ,string message ,string logger ,string exception = null)
    {
        try
        {
            using var connection = CreateConnection();
        ...
        {
            // إعادة القاء الخطأ ليتم التعامل معه في الطبقات الأعلى     
            throw new Exception("فشل في إضافة سجل إلى قاعدة البيانات" ,ex);
        }
    }
    ...
    public void LoadLogsFromDatabase()
    {
        using var connection = CreateConnection();
        connection.Open();
        var command = new SqlCommand("SELECT * FROM Logs ORDER BY TimeStamp DESC");
