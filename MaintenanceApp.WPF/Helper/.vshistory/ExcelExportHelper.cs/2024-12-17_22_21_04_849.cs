...
public static class ExcelExportHelper
{
    ...
    public static void ExportToExcelWithProtection(dynamic data ,string filePath ,string password)
    {
        try
        {
            ...
            {
                var worksheet = CreateWorksheet(workbook ,"Visit Data Report");
                AddHeaders(worksheet);
                AddData(worksheet ,data);
                FormatWorksheet(worksheet ,password);
        ...
        {
            Debug.WriteLine($"الملف مفتوح بالفعل: {ex.Message}");
        ...
        {
            Debug.WriteLine($"حدث خطأ أثناء التصدير: {ex.Message}");
    ...
    {
        cell.Style.Font.SetFontSize(14).SetBold().FontColor = XLColor.Black;
