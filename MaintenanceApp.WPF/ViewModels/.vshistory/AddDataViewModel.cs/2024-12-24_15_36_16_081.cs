...
public class AddDataViewModel : BaseViewModel
{
    ...
    public ObservableCollection<SAPData> Records { get; set; } = new();
    ...
    private bool FilterRecords(object item)
    {
        if(item is SAPData record)
            return string.IsNullOrEmpty(SearchQuery) ||
                   record.Notification?.IndexOf(SearchQuery ,StringComparison.OrdinalIgnoreCase) >= 0;
        return false;
    }
    ...
    private async Task LoadDataAsync()
    {
        try
        {
            ...
            var sapDataRecords = await _databaseHelper.GetAllRecordsAsync<SAPData>("SAPTable");
            Records.Clear();
            ...
            var anotherTableRecords = await _databaseHelper.GetAllRecordsAsync<AnotherTableModel>("AnotherTable");
            AnotherTableRecords.Clear();
            foreach(var record in anotherTableRecords)
            {
                AnotherTableRecords.Add(record);
            }
        }
        catch(Exception ex)
        {
