...
public class AddVisitDataViewModel : BaseViewModel
{
    ...
    public AddVisitDataViewModel(IDatabaseHelper databaseHelper)
    {
        ...
        RowSelectedCommand = new RelayCommand(_ => OnRowSelected());
    ...
    private void OnRowSelected()
    {
        if(SelectedSAPData != null)
        {
            ...
            // يمكنك هنا إضافة منطق إضافي إذا لزم الأمر
        }
    }
    ...
    private async void LoadVisitData()
    {
        try
        {
            ...
            //var sapDataList = await _databaseHelper.GetAllRecordsAsync<VisitData>();
            var sapDataList = await _databaseHelper.GetFilteredRecordsAsync<VisitData>("VisitDatas","Notification" ,SelectedSAPData.Notification.ToString());
            foreach(var sap in sapDataList)
            {
                VisitDataRecords.Add(sap);
            }
           
            OnPropertyChanged(nameof(VisitDataRecords));
            MessageBox.Show($"VisitDataRecords.Count: {VisitDataRecords.Count}");
        }
        catch(Exception ex)
        {
            MessageBox.Show($"Error loading data: {ex.Message}");
