...
public class AddVisitDataViewModel : BaseViewModel
{
    ...
    {
        EmployeeRecords = new ObservableCollection<EmployeeData>(_employeeDataViewModel.EmployeeRecords);
        ...
        else
        {
            FilteredEmployeeRecords = new ObservableCollection<EmployeeData>(
                EmployeeRecords.Where(e =>
                    (e.Name?.Contains(EmployeeSearchKeyword  ) == true) ||
                    e.Code.ToString().Contains(EmployeeSearchKeyword)));
        }
        OnPropertyChanged(nameof(FilteredEmployeeRecords));
    }
