...
public class AddVisitDataViewModel : BaseViewModel
{
    ...
    private SAPData _selectedSAPDataRecords;
    public SAPData SelectedSAPDataRecords
    {
    ...
    public AddVisitDataViewModel(IDatabaseHelper databaseHelper)
    {
        ...
        DistributeCommand = new RelayCommand(async _ => await DistributeAsync());
    ...
    {
        if(SelectedEmployee == null || VisitDate == null || SelectedSAPDataRecords == null || SelectedSAPDataRecords.Count == 0)
        {
            MessageController.SummaryAsync("يجب تحديد الفني وتاريخ الزيارة وصفوف البيانات.");
            return;
        }
    ...
    private async Task AddTechnicianVisitAsync()
    {
        ...
        if(SelectedSAPDataRecords == null)
        {
        ...
        try
        {
            var newVisit = new VisitData
            {
                Notification = SelectedSAPDataRecords?.Notification ?? throw new InvalidOperationException("SelectedSAPDataRecords is null.") ,
                VisitDate = VisitDate ,
        ...
        {
            MessageController.SummaryAsync($"حدث خطأ أثناء إضافة التوزيع: {ex.Message}");
        }
    }
    ...
    private async void LoadVisitData()
    {
        try
        {
            ...
            var linkedVisitData = await _databaseHelper.GetFilteredLinkedRecordsAsync(
                ...
                SelectedSAPDataRecords.Notification.ToString() // قيمة الإشعار المحدد
            );
    ...
    {
        if(SelectedSAPDataRecords == null) return; else MessageController.SummaryAsync("Not Select");

        try
        {
            var newVisitData = new VisitData
            {
                Notification = SelectedSAPDataRecords.Notification ,
            ...
            SelectedSAPDataRecords.Visits.Add(newVisitData);
            LoadVisitData();
        }
        catch(Exception ex)
        {
    ...
    private async Task DeleteVisitDataAsync()
    {
        ...
        try
        {
            ...
            SelectedSAPDataRecords.Visits.Remove(SelectedVisitData);
            LoadVisitData();
        }
        catch(Exception ex)
        {
