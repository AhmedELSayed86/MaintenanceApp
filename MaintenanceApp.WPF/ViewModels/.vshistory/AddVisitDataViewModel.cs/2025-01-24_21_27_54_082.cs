...
public class AddVisitDataViewModel : BaseViewModel
{
    ...
    public AddVisitDataViewModel(IDatabaseHelper databaseHelper)
    {
        ...
        AddSAPDataCommand = new RelayCommand(async ( ) => await AddSAPDataAsync());
        AddTechnicianVisitCommand = new RelayCommand(async () => await AddTechnicianVisitAsync());
        AddVisitDataCommand = new RelayCommand(async () => await AddVisitDataAsync());
        DeleteVisitDataCommand = new RelayCommand(async () => await DeleteVisitDataAsync());
        UpdateVisitDataCommand = new RelayCommand(async () => await UpdateVisitDataAsync());

        // الأمر الجديد للتوزيع
        DistributeCommand = new RelayCommand(async () => await DistributionToTechniciansAsync());
    ...
    private async Task DistributionToTechniciansAsync()
    {
        ...
        try
        {
            ...
            await MessageController.SummaryAsync("تمت التوزيع بنجاح.");
        }
        catch(Exception ex)
        {
    ...
    private async Task AddTechnicianVisitAsync()
    {
        ...
        try
        {
            var newVisit = new VisitData
            {
                ...
                VisitDate = VisitDate ,
                Technician = SelectedEmployee?.Code ?? throw new InvalidOperationException("SelectedEmployee is null.") ,
        ...
        {
            await MessageController.SummaryAsync($"حدث خطأ أثناء إضافة التوزيع: {ex.Message}");
        }
    }
    ...
    private async void ApplyFilter()
    {
        ...
        {
            FilteredSAPDataRecords = new ObservableCollection<SAPDataWithStatus>(SAPDataRecords);
            await MessageController.SummaryAsync($"ApplyFilterif FilteredSAPDataRecords.Count: {FilteredSAPDataRecords.Count}");
        }
        else
        {
            ...
            await MessageController.SummaryAsync($"ApplyFilterelse FilteredSAPDataRecords.Count: {FilteredSAPDataRecords.Count}");
        }
    }
    ...
    private async Task AddSAPDataAsync()
    {
        ...
        {
            await MessageController.SummaryAsync($"Error adding SAP data: {ex.Message}");
        }
    }
