...
public class DistributionToTechniciansViewModel : BaseViewModel
{
    ...
    {
        string table1 = nameof(VisitData);
        string table2 = nameof(SAPData);
        string table3 = nameof(EmployeeData);
        string joinColumn = nameof(VisitData.Notification);
        try
        {
            ...
            var linkedVisitData = await _databaseHelper.GetFilteredLinkedDateRecordsAsync(
                ...
                table3 , // الجدول الثالث
                joinColumn ,  // عمود الربط بين VisitDatas و SAPDatas
    ...
    {
        if(SelectedSAPData == null) return; else await MessageController.SummaryAsync("Not Select").ConfigureAwait(false);
