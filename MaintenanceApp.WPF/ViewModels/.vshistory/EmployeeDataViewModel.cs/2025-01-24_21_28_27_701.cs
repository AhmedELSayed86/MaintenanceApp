...
public class EmployeeDataViewModel : BaseViewModel
{
    ...
    public EmployeeDataViewModel(IDatabaseHelper databaseHelper)
    {
        ...
        LoadExcelEmployeesCommand = new RelayCommand(async () => await LoadEmployeeDataFromExcelAsync());
        ImportEmployeesCommand = new RelayCommand(async () => await SaveEmployeeDataToDatabaseAsync());
        LoadDataCommand = new RelayCommand(async () => await LoadDataAsync());
        AddEmployeeCommand = new RelayCommand(async () => await AddEmployeeAsync());
        UpdateEmployeeCommand = new RelayCommand(async () => await UpdateEmployeeAsync());
        ...
        try
        {
            ...
            if(ExcelData != null && ExcelData.Rows.Count > 0)
            {
                ...
                OnPropertyChanged(nameof(EmployeeRecords));               
                              ...
                              Summary = $"تم تحميل {ExcelData.Rows.Count} صفوف من ملف اكسيل.({EmployeeRecords.Count})";
            }
            else
            {
    ...
    public async Task SaveEmployeeDataToDatabaseAsync()
    {
        ...
        foreach(DataRow row in ExcelData.Rows)
        {
            try
            {
                ...
                {
                    if(await _databaseHelper.CheckIfSapCodeExistsAsync("EmployeeDatas" ,"Code" ,parameters["Code"].ToString()))
                    {
                        skippedExists++;
                    }
                    else
                    {
                        await _databaseHelper.ExcelInsertIntoTableAsync("EmployeeDatas" ,parameters);
                        addedRows++;
                    }
                }
                else
