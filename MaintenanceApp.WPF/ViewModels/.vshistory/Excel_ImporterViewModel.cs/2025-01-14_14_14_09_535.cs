...
{ 
        ...
        public StatusViewModel(IDatabaseHelper databaseHelper)
        {
            _databaseHelper = databaseHelper;

            LoadExcelStatussCommand = new RelayCommand(async _ => await LoadStatusFromExcelAsync());
            ImportStatussCommand = new RelayCommand(async _ => await SaveStatusToDatabaseAsync());
        ...
        public async Task SaveStatusToDatabaseAsync()
        {
            ...
            foreach(DataRow row in ExcelData.Rows)
            {
                try
                {
                    ...
                    {
                        if(await _databaseHelper.CheckIfSapCodeExistsAsync("Statuss" ,"Code" ,Convert.ToDouble(parameters["Code"])))
                        {
                            skippedExists++;
                        }
                        else
                        {
                            await _databaseHelper.ExcelInsertIntoTableAsync("Statuss" ,parameters);
                            addedRows++;
                        }
                    }
                    else
