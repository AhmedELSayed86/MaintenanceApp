...
public class ImportExcelViewModel : BaseViewModel
{
    ...
    public ImportExcelViewModel(IDatabaseHelper databaseHelper ,Action onClose)
    {
        ...
        LoadExcelCommand = new DelegateCommand(async () => await LoadExcelAsync());
        SaveToDatabaseCommand = new DelegateCommand(async () => await SaveToDatabaseAsync() ,CanSaveToDatabase)
                                 .ObservesProperty(() => ExcelData);
        CloseCommand = new DelegateCommand(() => _onClose());
    }
