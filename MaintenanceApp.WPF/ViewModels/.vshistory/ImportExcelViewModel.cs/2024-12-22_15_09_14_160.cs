...
public class ImportExcelViewModel : BaseViewModel
{
    ...
    private List<dynamic> _excelDataList;
    public List<dynamic> ExcelDataList
    {
        get => _excelDataList;
        set
        {
    ...
    private async Task LoadExcelAsync()
    {
        try
        {
            ...
            if(ExcelData != null && ExcelData.Rows.Count > 0)
            {
                ExcelDataList = ExcelData.AsEnumerable()
                    .Select(row => new
                    {
    ...
    private async Task SaveToDatabaseAsync()
    {
        ...
        foreach(DataRow row in ExcelData.Rows)
        {
            try
            {
                ...
                {
                    if(await _databaseHelper.CheckIfSapCodeExistsAsync("SAPData" ,Convert.ToDouble(parameters["Notification"])))
                    {
                        skippedExists++;
                    }
                    else
                    {
                        await _databaseHelper.InsertIntoTableAsync("SAPData" ,parameters);
                        addedRows++;
                    }
                }
                else
