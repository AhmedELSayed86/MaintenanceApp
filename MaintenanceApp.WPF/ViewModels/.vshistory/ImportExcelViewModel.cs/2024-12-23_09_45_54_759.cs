...
public class ImportExcelViewModel : BaseViewModel
{
    ...
    private async Task LoadExcelAsync()
    {
        try
        {
         ...
         {
             Notification = Convert.ToDouble(row["Notification"].ToString()) ,
             NotificationType = row["NotificationType"]?.ToString() ,
             Region = Convert.ToInt32(row["Region"]?.ToString()) ,
             City = row["City"]?.ToString() ,
             Street = row["Street"]?.ToString() ,
             ListName = row["ListName"]?.ToString() ,
             Telephone = row["Telephone"]?.ToString() ,
             District = row["District"]?.ToString() ,
             NotifDate = Convert.ToDateTime(row["NotifDate"]?.ToString()) ,
             Description = row["Description"]?.ToString() ,
             Customer = Convert.ToInt32(row["Customer"]?.ToString()) ,
             MainWorkCtr = row["MainWorkCtr"]?.ToString() ,
             SortField = row["SortField"]?.ToString() ,
             BreakdownDuration = Convert.ToInt32(row["BreakdownDuration"]?.ToString()) ,
             RequiredEnd = Convert.ToDateTime(row["RequiredEnd"]?.ToString()) ,
         }));
            }
