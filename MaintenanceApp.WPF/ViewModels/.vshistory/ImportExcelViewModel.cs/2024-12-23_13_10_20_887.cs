...
public class ImportExcelViewModel : BaseViewModel
{
    ...
    private async Task LoadExcelAsync()
    {
        try
        {
            ...
            MessageBox.Show($"تم تحميل البيانات بنجاح ExcelDataList: {ExcelDataList.Count} صفوف.");
            MessageBox.Show($"تم تحميل البيانات بنجاح FilteredExcelDataList: {FilteredExcelDataList.Count} صفوف.");
        }
        catch(Exception ex)
        {
