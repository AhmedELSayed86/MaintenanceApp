...
public class ImportExcelViewModel : BaseViewModel
{
    ...
    private async Task LoadExcelAsync()
    {
        ...
        {
            IsLoading = true;
            ExcelData = await ExcelHelper.LoadFromExcelAsync(ExcelHelper.FilePathSAPDetails);
        ...
        try
        {
            ...
            if(ExcelData != null && ExcelData.Rows.Count > 0)
            {
                var employeeDataList = ExcelData.AsEnumerable().Select(row => new EmployeeData
                {
                    ...
                    ChangeOn = row["ChangeOn"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(row["ChangeOn"]) ,
                    CreatedBy = row["CreatedBy"] == DBNull.Value ? 0 : Convert.ToInt32(row["CreatedBy"]) ,
                    ChangeBy = row["ChangeBy"] == DBNull.Value ? 0 : Convert.ToInt32(row["ChangeBy"]) ,
