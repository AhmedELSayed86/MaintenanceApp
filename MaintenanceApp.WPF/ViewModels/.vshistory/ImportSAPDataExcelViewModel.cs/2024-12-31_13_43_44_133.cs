...
public class ImportSAPDataExcelViewModel : BaseViewModel
{
    private readonly IDatabaseHelper _databaseHelper;
    private readonly EmployeeDataViewModel _employeeDataViewModel;

    private ObservableCollection<string> _employeeNames;
    ...
    public ImportSAPDataExcelViewModel(IDatabaseHelper databaseHelper)
    {
    ...
    {
        if(_employeeDataViewModel.EmployeeRecords != null)
        {
            EmployeeNames = new ObservableCollection<string>(
                _employeeDataViewModel.EmployeeRecords.Select(e => e.Name).Where(name => !string.IsNullOrEmpty(name)));
        }
    }
    ...
    private async Task SaveSAPDataToDatabaseAsync()
    {
        ...
        foreach(DataRow row in ExcelData.Rows)
        {
            try
            {
                ...
                {
                    if(await _databaseHelper.CheckIfSapCodeExistsAsync("SAPDatas" ,"Notification" ,Convert.ToDouble(parameters["Notification"])))
                    {
                        skippedExists++;
                    }
                    else
