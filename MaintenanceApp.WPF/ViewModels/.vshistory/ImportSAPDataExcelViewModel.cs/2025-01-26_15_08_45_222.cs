...
public class ImportSAPDataExcelViewModel : BaseViewModel
{
    ...
    public ImportSAPDataExcelViewModel(IDatabaseHelper databaseHelper)
    {
        ...
        await LogInfoAsync("تم تهيئة ImportSAPDataExcelViewModel بنجاح.");
    }
    ...
    private async Task SaveSAPDataToDatabaseAsync()
    {
        ...
        foreach(DataRow row in ExcelData.Rows)
        {
            try
            {
                ...
                {
                    if(await _databaseHelper.CheckIfSapCodeExistsAsync("SAPDatas" ,"Notification" ,parameters["Notification"].ToString()))
                    {
                        skippedExists++;
                    }
                    else
                    {
                        await _databaseHelper.ExcelInsertIntoTableAsync("SAPDatas" ,parameters);
                        addedRows++;
                    }
                }
                else
